...
function math.getAlpha(dist,p)
	p=p or 15
	local alpha=255-p*dist
...
function math.getFirstCar()
	local maxLap=-1
	local maxPos=-1
	local car
	for k,v in pairs(game.npc) do
		if v.lap>=maxLap then
			maxLap=v.lap
			if v.posInRoad>=maxPos then
				maxPos=v.posInRoad
				car=game.npc[k]
			end
		end
	...
	local v=game.car
	if v.lap>=maxLap and v.posInRoad>=maxPos then
		car=game.car
	end
	return car
end
